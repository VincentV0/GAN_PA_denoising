function data_arr = patch_extract(img,patch_size)
    ...
    data_arr = zeros(size(img,1)*patches_x*patches_y, patch_size(1), patch_size(2));
    i = 1;
    for x=0:patches_x-1
        for y=0:patches_y-1
            data_arr(i, :, :) = img(1, patch_size(1)*x+1:patch_size(1)*(x+1) , patch_size(2)*y+1:patch_size(2)*(y+1));
            i = i+1;
        end
    end
end
